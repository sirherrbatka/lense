...
(docs:define-docs
  ...
  (type composed-lense
    (:description "COMPOSED-LENSE pipes all READ, WRITE and COMPOSE calls trough the lenses used to construct the result (from left to right, in the opposite order ALEXANDRIA:COMPOSE works)."))
  ...
  (function gather
    (:exceptional-situations "Will signal CL:TYPE-ERROR (with interactive restart) if DESTINATION-VECTOR is not (AND VECTOR (SATISFIES ADJUSTABLE-ARRAY-P))"
     :DESCRIPTION "This function gathers all elementary lenses from the input lense. This is required for the simplicity and efficiency of the COMPOSE function. Constructed COMPOSED-LENSE has flat internal lenses structure which eleminates pointless recursive calls that can potentially occur otherwise."
     :arguments-and-values ((lense "LENS being scaned.")
                            (destination-vector "Vector. Function will call vector-push-extend to insert relevant content into it."))
